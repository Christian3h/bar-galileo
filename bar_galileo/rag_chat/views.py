...
from .models import DocumentChunk, DocumentCollection, RAGQuery
    ...
    def post(self, request):
        try:
            ...
            if not file.name.lower().endswith(".pdf"):
                return JsonResponse(
                    {"error": "Solo se admiten archivos PDF"}, status=400
                )
...
class QueryRAGView(View):
    ...
    def post(self, request):
        try:
            ...
            generator = get_embedding_generator()
            # Si es TF-IDF y no está ajustado, ajustarlo con los textos de la colección
            ...
            # 3. Generar respuesta con el proveedor LLM seleccionado
