...
const initWebSocket = () => {
  ...
  const ws_path =
    ws_scheme + "://" + window.location.host + "/ws/notificaciones/";
...
const mostrarMenosProductos = () => {
  ...
  const productosAdicionales = productosGrid.querySelectorAll(
    ".producto-adicional",
  );
...
const initHeroCarousel = () => {
  // Only initialize hero carousel on pages that have a hero section
  const heroSection = document.querySelector(".hero");
  if (!heroSection) {
    // No hero section on this page, skip initialization silently
    return;
  }
  ...
  if (slides.length === 0) {
    // Hero section exists but slides not found - this shouldn't happen normally
    console.warn("⚠️ Hero section found but no slides - check HTML structure");
    return;
  }
  ...
  // Pause carousel on hover (heroSection already defined above)
