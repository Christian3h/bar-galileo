...
class ProductoForm(forms.ModelForm):
    ...
    def clean_nombre(self):
        ...
        if self.instance.pk:
            if (
                Producto.objects.exclude(pk=self.instance.pk)
                .filter(nombre__iexact=nombre)
                .exists()
            ):
                raise forms.ValidationError(
                    "Ya existe un producto con este nombre. Verifica antes de continuar."
                )
        else:
    ...
    def clean_precio_venta(self):
        ...
        if precio_venta > 1000000000:  # Límite razonable
            raise forms.ValidationError(
                "El precio de venta no puede superar $1,000,000,000."
            )
        return precio_venta
    ...
    def clean_telefono(self):
        ...
        if telefono:
            ...
            if not re.match(colombian_phone_regex, telefono_str):
                raise forms.ValidationError(
                    "El número de teléfono debe ser colombiano (10 dígitos, que empiece con 3)."
                )
        return telefono
